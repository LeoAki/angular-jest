...
describe('Suite UserService', () => {
  ...
  beforeAll(() => {
    console.log('called beforeAll spec are run');
  })
  ...
  beforeEach(() => {
    ...
    httpClientMock.get.mockReturnValue(of(usersListMock));
    console.log('called beforeEach spec is run');
  });
  ...
  it('getUser return UserList', (done) => {
    service.getUser().subscribe((res) => {
      expect(res.users.length).toBe(30);
      done();
    });
  });

  it('getUser returns username', (done) => {
    httpClientMock.get.mockReturnValue(of(usersListMock));
    const expectUsername = 'emilys';
    service.getUser().subscribe((resp) => {
      expect(resp.users[0].username).toEqual(expectUsername);
      done();
    })
  })
});
